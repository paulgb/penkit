...
def make_noise_surface(dims=DEFAULT_DIMS, blur=10, seed=None):
    ...
    Params:
    - dims: the dimensions of the surface to create
    - blur: the amount of Gaussian blur to apply
    - seed: a random seed to use (optional)
    """
    if seed is not None:
        np.random.seed(seed)
...
def make_sine_surface(dims=DEFAULT_DIMS, offset=0.5, scale=1.0):
    ...
    Params:
    - dims: the dimensions of the surface to create
    - offset: an offset applied to the function
    - scale: a scale applied to the sine frequency
...
def make_bubble_surface(dims=DEFAULT_DIMS, repeat=3):
    ...
    Params:
    - dims: the dimensions of the surface to create
    - repeat: the frequency of the waves is set to ensure this many
